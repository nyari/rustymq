...
impl ReceiptState {
    ...
    pub fn has_been_received(&self) -> Result<bool, Self> {
        match self {
            ReceiptState::Received | ReceiptState::Acknowledged | ReceiptState::Processed => {
                Ok(true)
            }
    ...
    fn transition(&mut self, state: Self) {
        ...
        *self = new_state.expect(
            format!(
                "Invalid ReceiptState transition. From: {:?}, To: {:?}",
                self, state
            )
            .as_str(),
        );
...
impl ReceiptInternalData {
    fn queue() -> (SenderReceipt, ReceiverReceipt) {
        ...
        (
            SenderReceipt::new(internals.clone()),
            ReceiverReceipt::new(internals),
        )
    }
}
...
impl ReceiverReceipt {
    ...
    pub fn received(self) -> Self {
        self.0
            ...
            .lock_notify()
            .unwrap()
            .transition(ReceiptState::Received);
        self
    }
    ...
    pub fn acnkowledged(self) {
        self.0
            ...
            .lock_notify()
            .unwrap()
            .transition(ReceiptState::Acknowledged);
    }
    ...
    pub fn processed(self) {
        self.0
            ...
            .lock_notify()
            .unwrap()
            .transition(ReceiptState::Processed);
    }
}
...
impl Drop for ReceiverReceipt {
    fn drop(&mut self) {
        self.0
            ...
            .lock_notify()
            .unwrap()
            .transition(ReceiptState::Dropped);
    }
}
    ...
    T: Send + Sync,
{
    ...
    fn receive_with_receipt(&self) -> Result<(Option<ReceiverReceipt>, T), MessageQueueError> {
        ...
        loop {
            match locked.queue.pop_front() {
                Some((receipt, message)) => return Ok((receipt.map(|x| x.received()), message)),
                None => {
                    if locked.senders_all_dropped() {
                        return Err(MessageQueueError::SendersAllDropped);
                    }
    ...
    fn receive_timeout(&self, timeout: Duration) -> Result<T, MessageQueueError> {
        ...
        for _ in 0..2 {
            ...
            locked = self
                .0
                .wait_timeout_on_lock_notified(locked, timeout)
                .unwrap()
                .0
        }
    ...
    fn receive_timeout_with_receipt(
        &self,
        timeout: Duration,
    ) -> Result<(Option<ReceiverReceipt>, T), MessageQueueError> {
        let mut locked = self.0.lock_notify().unwrap();
        for _ in 0..2 {
            match locked.queue.pop_front() {
                Some((receipt, message)) => return Ok((receipt.map(|x| x.received()), message)),
    ...
    fn receive_async_with_receipt(
        &self,
    ) -> Result<Option<(Option<ReceiverReceipt>, T)>, MessageQueueError> {
        let mut locked = self.0.lock_notify().unwrap();
        match locked.queue.pop_front() {
            Some((receipt, message)) => Ok(Some((receipt.map(|x| x.received()), message))),
            None => {
                if locked.senders_all_dropped() {
                    Err(MessageQueueError::SendersAllDropped)
                } else {
    ...
    fn receive_all_with_receipt(
        &self,
    ) -> Result<Vec<(Option<ReceiverReceipt>, T)>, MessageQueueError> {
        let result: Vec<(Option<ReceiverReceipt>, T)> = self
            .0
            .lock_notify()
            .unwrap()
            .queue
            .drain(..)
            .map(|(receipt, message)| match receipt {
        ...
        loop {
            ...
            if !locked.is_queue_full() {
                ...
                match locked.get_overflow_handling() {
                    ...
                    Some(MessageQueueOverflowHandling::ErrorAndDrop) => {
    ...
    fn send_with_receipt(&self, message: T) -> Result<SenderReceipt, MessageQueueError> {
        ...
        loop {
            ...
            if !locked.is_queue_full() {
                ...
                match locked.get_overflow_handling() {
                    Some(MessageQueueOverflowHandling::Throttle) => (),
                    Some(MessageQueueOverflowHandling::Drop) => {
                        return Err(MessageQueueError::Dropped)
                    }
                    Some(MessageQueueOverflowHandling::ErrorAndDrop) => {
                        return Err(MessageQueueError::QueueFull)
                    }
...
#[derive(Clone)]
pub struct MessageQueueReceiver<T>(MessageQueueInternal<T>)
where
    T: Send + Sync;
    ...
    T: Send + Sync,
{
    ...
    pub fn receive_timeout_with_receipt(
        &self,
        timeout: Duration,
    ) -> Result<(Option<ReceiverReceipt>, T), MessageQueueError> {
    ...
    pub fn receive_async_with_receipt(
        &self,
    ) -> Result<Option<(Option<ReceiverReceipt>, T)>, MessageQueueError> {
    ...
    pub fn receive_all_with_receipt(
        &self,
    ) -> Result<Vec<(Option<ReceiverReceipt>, T)>, MessageQueueError> {
...
#[derive(Clone)]
pub struct MessageQueueSender<T>(MessageQueueInternal<T>)
where
    T: Send + Sync;
